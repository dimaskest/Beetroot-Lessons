...
def menu_update_contact_info(phonebook):
    number = input("Please enter number of a contact you would like to update: ")
    key = input("Please enter an option you would like to update (first_name, last_name full_name, city, state): ")
        ...
        try:
            ...
            if menu_choice != 0:
                if not menu_choice in menu_func.keys():
                    print("There is no such menu choice, please choose the correct number: ")
                    print_menu()
                    break
                else:
